...
def check_relay_health() -> dict | None:
    try:
        req = urllib.request.Request(f"{BASE_URL}/serving", headers={"User-Agent": "AURALauncher/1.1.4"})
...
def start_relay(log: Callable[[str], None] = print) -> bool:
    ...
    if health and health.get("ok"):
        log(f"Relay läuft bereits auf {BASE_URL} (v{health.get('version', '1.1.4')}).")
        return True
    ...
    while time.time() < deadline:
        ...
        health = check_relay_health()
        if health and health.get("ok"):
            log(f"Relay bereit: {BASE_URL} (v{health.get('version', '1.1.4')}).")
            return True
        time.sleep(0.2)
...
def print_banner():
    print("\n" + "=" * 60)
    print("       AURA Quant Terminal — Quant Research & Setup Discovery v1.1.4")
    print("=" * 60)
