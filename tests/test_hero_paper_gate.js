...
assert(heroSource.includes('canStartHeroPaperTrade(L)'), 'renderHero must use the paper-trade gate');
assert(heroSource.includes('btnPaper.disabled'), 'renderHero must disable the blocked Paper button');
...
assert(paperSource.includes('if (!canStartHeroPaperTrade(L)) return;'), 'Paper start handler must independently fail closed');
